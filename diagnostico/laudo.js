...
// Montagem
// ---------------------------------------------------------------------------
function montarLaudo(bruto, { testes = null, momento = "entrada", os = null, tecnico = "", observacao = "", agente = "" } = {}) {
  const b = bruto || {};
  ...
  if (!laudo.administrador) {
    laudo.limitacoes.push("Coleta sem permissão de administrador: saúde SMART dos discos (desgaste, horas ligado, erros) e temperaturas podem não aparecer.");
  }
...
function gerarAlertas(l) {
  ...
  for (const d of l.discos || []) {
    ...
    if (d.saude && d.saude !== "Healthy") add("critico", "Disco", `${nome}: saúde "${d.saude}"`, "O Windows indica falha ou risco de falha. Faça backup imediato e avalie a troca.");
  ...
  for (const v of l.volumes || []) {
    ...
    if (v.livrePct < 5) add("critico", "Armazenamento", `Unidade ${v.letra}: com ${v.livreGB} GB livres (${v.livrePct}%)`, "Pouco espaço deixa o sistema lento e pode impedir atualizações.");
    else if (v.livrePct < 15) add("atencao", "Armazenamento", `Unidade ${v.letra}: com ${v.livreGB} GB livres (${v.livrePct}%)`, "Recomenda-se liberar espaço.");
  }
  if (l.bateria) {
  ...
  const mem = l.memoria || {};
  if (mem.totalGB && mem.totalGB < 4) add("atencao", "Memória", `${mem.totalGB} GB de memória RAM`, "Pouca memória para o Windows atual; considere ampliar.");
  ...
  if (l.sistema && !(l.sistema.antivirus || []).length) add("atencao", "Segurança", "Nenhum antivírus registrado", "");
  ...
  if (l.limitacoes?.length) add("info", "Coleta", "Coleta sem administrador", l.limitacoes[0]);
...
function comparar(entrada, saida) {
  ...
  for (const v of entrada.volumes || []) {
    ...
    if (n) add("Armazenamento", `Unidade ${v.letra}: livre (GB)`, v.livreGB, n.livreGB, "maior");
  }
  add("Bateria", "Saúde (%)", entrada.bateria?.saudePct, saida.bateria?.saudePct, "maior");
  add("Sistema", "Windows", entrada.sistema?.nome, saida.sistema?.nome, null);
...
module.exports = { FORMATO, VERSAO, montarLaudo, gerarAlertas, situacaoGeral, validarLaudo, hashDe, selar, comparar, mesmoEquipamento, resumo };
