...
const SECOES = [
  ...
  },
  {
    ...
    campos: [
      ...
      t("linkSuporte", "Link do suporte (topo e rodapé)", { max: 30 }),
      t("linkEmissores", "Link do guia de emissores de nota (rodapé)", { max: 40 }),
    ...
    padrao: {
      ...
      linkSuporte: "Suporte",
      linkEmissores: "Emissores de nota fiscal",
  ...
  {
    chave: "pagina.suporte",
    grupo: "paginas",
    titulo: "Suporte",
    campos: [
      t("titulo", "Título", { max: 80 }),
      area("texto", "Instruções", { max: 600 }),
      area("aviso", "Aviso abaixo do formulário (prazo de resposta etc.)", { max: 400 }),
      t("enviadoTitulo", "Confirmação · título", { max: 80 }),
      area("enviadoTexto", "Confirmação · texto", { max: 400 }),
    ],
    padrao: {
      titulo: "Como podemos ajudar?",
      texto: "Conte o que aconteceu: dúvidas, erros, sugestões ou questões de pagamento. Quanto mais detalhes (e prints da tela), mais rápido resolvemos. Clientes podem abrir chamados também pelo próprio sistema, em Suporte.",
      aviso: "Respondemos em dias úteis, normalmente em até 1 dia útil. Você recebe cada resposta por e-mail.",
      enviadoTitulo: "Chamado aberto",
      enviadoTexto: "Enviamos o número e o link de acompanhamento para o seu e-mail. Responda por lá ou pela página do chamado.",
    },
  },
  {
    chave: "pagina.emissores",
    grupo: "paginas",
    titulo: "Emissores de nota fiscal",
    campos: [
      t("titulo", "Título", { max: 80 }),
      area("texto", "Introdução", { max: 800 }),
      area("aviso", "Aviso sobre planos", { max: 400 }),
      area("solicitarTexto", "Chamada para pedir outro emissor", { max: 400 }),
    ],
    padrao: {
      titulo: "Emissores de nota fiscal",
      texto: "O GSTI App não é um emissor: ele registra a nota que você emite por fora ou envia os dados da OS para o emissor que você escolher. A conta, o contrato e o certificado digital no emissor são seus. Veja abaixo como configurar cada opção.",
      aviso: "O registro manual está em todos os planos. A emissão pelo app com emissor integrado está no plano Anual com renovação automática.",
      solicitarTexto: "Usa outro emissor ou o portal da sua prefeitura? Peça a integração na área do cliente (ou pelo próprio sistema, em Configurações → Nota fiscal). Os mais pedidos entram primeiro.",
    },
  },
  {
    chave: "pagina.termos",
...
SECOES.push(
  ...
  email("chamado_aberto", "Suporte · chamado aberto (para o cliente)", "{{numero}}, {{assunto}}, {{link}}",
    "Chamado #{{numero}} aberto · {{produto}}",
    "Olá, {{nome}}!\n\nRecebemos seu chamado **#{{numero}} · {{assunto}}**. Nossa equipe vai analisar e responder por aqui.\n\n[Acompanhar o chamado]({{link}})\n\nPara acrescentar informações ou prints, use a página do chamado."),
  email("chamado_respondido", "Suporte · nova resposta (para o cliente)", "{{numero}}, {{assunto}}, {{resposta}}, {{link}}",
    "Resposta no chamado #{{numero}} · {{produto}}",
    "Olá, {{nome}}!\n\nHá uma nova resposta no chamado **#{{numero}} · {{assunto}}**:\n\n{{resposta}}\n\n[Ver e responder]({{link}})"),
  email("chamado_equipe", "Suporte · aviso para a equipe", "{{numero}}, {{assunto}}, {{evento}}, {{categoria}}, {{mensagem}}, {{link_painel}}",
    "[Suporte] {{evento}} · #{{numero}} {{assunto}}",
    "**{{evento}}** no chamado #{{numero}} ({{categoria}})\n\nDe: {{nome}} · {{email}}\n\n{{mensagem}}\n\n[Abrir no painel]({{link_painel}})"),
  email("emissor_disponivel", "Emissor de nota fiscal disponível", "{{emissor}}, {{link}}",
    "{{emissor}} chegou ao {{produto}}",
    "Olá, {{nome}}!\n\nVocê pediu a integração com **{{emissor}}** e ela já está disponível no {{produto}}.\n\nAtualize o sistema e configure em **Configurações → Nota fiscal**. O passo a passo está no [guia de emissores]({{link}}).\n\nObrigado por ajudar a decidir as próximas integrações!"),
