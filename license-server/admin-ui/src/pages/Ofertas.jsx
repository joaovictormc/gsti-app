...
function EditarOferta({ oferta, onFechar, onSalva }) {
  ...
  const salvar = async () => {
    ...
    try {
      await put(`/ofertas/${oferta.id}`, {
        ...
        maxMaquinas: Number(f.maxMaquinas), destaque: f.destaque, ativo: f.ativo, ordem: Number(f.ordem),
      });
      onSalva();
    } catch (e) {
      setErro(e.message);
    ...
    <Dialog open onClose={() => !ocupado && onFechar()} maxWidth="sm" fullWidth>
      ...
      <DialogContent>
        <Stack spacing={2} sx={{ mt: 1 }}>
          ...
          <Alert severity="info">Mudanças de preço valem para novos pedidos. Renovações automáticas já contratadas mantêm o valor original.</Alert>
    ...
    <>
      ...
      {carregando ? <Carregando /> : dados && (
        <Paper>
          <TableContainer>
            <Table>
              <TableHead><TableRow><TableCell>Plano</TableCell><TableCell align="right">Preço</TableCell><TableCell>Pagamento</TableCell><TableCell>Computadores</TableCell><TableCell>No site</TableCell><TableCell /></TableRow></TableHead>
              <TableBody>
                {dados.itens.map((o) => (
                  <TableRow key={o.id}>
                    <TableCell>
      ...
      {editando && <EditarOferta oferta={editando} onFechar={() => setEditando(null)} onSalva={() => { setEditando(null); avisar("Plano atualizado."); recarregar(); }} />}
      {aviso}
    </>
  );
}
