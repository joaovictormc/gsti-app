...
export default function NotasFiscaisOS({ os, onClose, onAlterado }) {
  ...
  const confirmarCancelamento = async () => {
    ...
    if (r.success) {
      setCancelando(null);
      setMensagem({ tipo: "success", texto: "Nota marcada como cancelada. Lembre-se de cancelá-la também onde foi emitida." });
      await carregar();
      onAlterado?.();
    } else {
      setMensagem({ tipo: "error", texto: r.error });
  ...
  return (
    <Dialog open={!!os} onClose={onClose} maxWidth="md" fullWidth>
        ...
        {status && (
          <Alert severity="info" sx={{ mb: 2 }}>
            {status.integradoAtivo ? (
              <>Emissor configurado: <strong>{status.emissor.nome}</strong>.</>
            ) : (
              <>
                  ...
                  <TableRow key={n.id}>
                    ...
                    <TableCell>
                      ...
                      <Typography variant="caption" color="text.secondary" sx={{ display: "block" }}>
                        {n.origem === "manual" ? "Registro manual" : n.origem}{n.usuario ? ` · ${n.usuario}` : ""}
                      </Typography>
                    </TableCell>
                    ...
                    <TableCell>
                      <Chip
                        size="small"
                        label={n.status === "cancelada" ? "Cancelada" : "Emitida"}
                        color={n.status === "cancelada" ? "default" : "success"}
                        variant={n.status === "cancelada" ? "outlined" : "filled"}
                      />
                    ...
                    <TableCell align="right" sx={{ whiteSpace: "nowrap" }}>
                      ...
                      {podeRegistrar && n.status !== "cancelada" && n.origem === "manual" && (
                        <IconButton size="small" title="Marcar como cancelada" onClick={() => setCancelando({ id: n.id, motivo: "" })}>
                          <BlockIcon fontSize="small" />
                        </IconButton>
                      )}
                      {podeRegistrar && permissoes.podeExcluir && n.origem === "manual" && (
      ...
      <DialogActions>
        {podeRegistrar && finalizada && !form && (
          <Button variant="contained" onClick={() => setForm(novaNota(os))}>Registrar nota</Button>
        )}
        {form && (
          <>
        ...
        {!form && <Button onClick={onClose}>Fechar</Button>}
      </DialogActions>
